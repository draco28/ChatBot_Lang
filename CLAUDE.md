# ChatBot_Lang - AI Workflow Guide
...
```
projectpulse_context_load(projectId: 8)
```
...
─────────────────────────────────────────────────
projectpulse_sprint_getCurrentPosition(projectId: 8)
...
projectpulse_persona_list(projectId: 8)

# Load a specific persona
projectpulse_persona_get(projectId: 8, slug: "backend-developer")
```
...
projectpulse_skill_list(projectId: 8, category: "framework")

# Load a skill
projectpulse_skill_get(projectId: 8, slug: "react-hooks-patterns")
```
...
```
# List available SOPs
projectpulse_sop_list(projectId: 8, category: "Development")

# Load an SOP
projectpulse_sop_get(projectId: 8, slug: "git-workflow")
```
...
|---------|------|-----------|
| DevOps Engineer | `devops-engineer` | Docker, Docker Compose, Health checks, CI/CD, Monitoring, Infrastructure as Code |
| LangChain AI Engineer | `langchain-engineer` | LangChain, AgentExecutor, Tools, Ollama, Callbacks, Chains, Memory |
| Python Backend Developer | `python-backend` | FastAPI, Pydantic, async/await, WebSocket, REST APIs, SQLAlchemy |
| QA Engineer | `qa-engineer` | pytest, Mocking, LLM testing, Integration testing, Test automation, Coverage analysis |
| RAG & Vector Specialist | `rag-specialist` | Qdrant, Embeddings, Chunking, RAG pipelines, Semantic search, Document processing |
| Security Engineer | `security-engineer` | Input validation, Prompt injection prevention, OWASP Top 10, Authentication, Authorization, Secrets management |
...
projectpulse_knowledge_search(projectId: 8, query: "authentication")

# Store new knowledge
projectpulse_knowledge_create(projectId: 8, title: "...", content: "...", category: "...")
```
...
- [ ] Loaded context via `context_load(projectId: 8)`
...
View all project resources: https://projectpulse.dracodev.dev/projects/8
